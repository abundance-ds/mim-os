...
describe('electron-builder updater configuration', () => {
  ...
  it('keeps CI manual-publish only while uploading builder metadata', async () => {
    ...
    expect(workflow).toContain('node scripts/verify-update-artifacts.mjs dist-installers "${{ matrix.update_manifest }}"')
    expect(workflow).not.toContain('"$UPDATE_MANIFEST"')
